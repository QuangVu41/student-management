...
if ($_SESSION['admin'] && $_SESSION['admin']['role_id'] == 1) {
    if(isset($_POST["update"])){
        ...
        $sql = "UPDATE `admin` SET admin_name = '$admin_name', date_of_birth = '$date_of_birth', phonenumber = $phonenumber,email = '$email', `address` = '$address', user_name = '$user_name',`password`= '$password', role_id = $role_id WHERE admin_id = '$admin_id'";
...
<body>
    ...
    <div class="container-fluid">
        <h2 class="cart-info__heading">Cập nhật tài khoản admin</h2>
        <form method = "post" id = "form">
        <label>Admin Id: </label><select name="admin_id" class="form-control w-25" style = "height: 35px;">
                    <?php
                        $sql1 = "SELECT admin_id, admin_name FROM `admin`";
                        $result = $conn->query($sql1);
                        while ($row = $result->fetch_assoc()) {
                        $admin_id = $row['admin_id']; 
                        $admin_name = $row["admin_name"];
                        echo "<option value='$admin_id'>". $admin_id . ": " . $admin_name . "</option>";
                        }
                    ?>
        </select><br/>
        <label style = "font-size: 17px;">Admin Name: </label><input type="text" name="admin_name" id = "admin_name" placeholder="Admin Name" style = "height: 35px;" class="form-control w-25"><br/>
            ...
            <input type="submit" id = "submit" class = "btn btn-susscess" name = "update" value="Update"/>
        </form>
    </div>
...
<?php } else {
    ...
    echo "Bạn không có quyền truy cập vào trang web này";
    $conn->close();
} ?>         
